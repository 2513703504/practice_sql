...
INSERT INTO Sales
VALUES ('浙江杭州XX店', '2022-03-03', '10001', '001', 20, 4000),
...
SELECT SUBSTRING(store_name, 1, 2) AS store, SUM(amt) AS allSale
FROM Sales
GROUP BY SUBSTRING(store_name, 1, 2);

SELECT SUBSTRING(Sales.store_name, 3, 2) AS city,
       tmp.province,
       Sales.sale_date,
       Sales.amt,
       tmp.allSale,
       Sales.amt / tmp.allSale           AS rate
FROM Sales,
     (SELECT SUBSTRING(store_name, 1, 2) AS province, SUM(amt) AS allSale
      FROM Sales
      GROUP BY SUBSTRING(store_name, 1, 2)) AS tmp
WHERE tmp.province = SUBSTRING(Sales.store_name, 1, 2);

SELECT tmp2.city, tmp2.province, tmp2.amt, tmp2.allSale, tmp2.rate
FROM (SELECT SUBSTRING(Sales.store_name, 3, 2) AS city,
             tmp.province,
             Sales.sale_date,
             Sales.amt,
             tmp.allSale,
...
WHERE tmp2.sale_date > '2022-02-29'
  AND tmp2.sale_date < '2022-04-01'
  AND tmp2.rate >= 0.1;
